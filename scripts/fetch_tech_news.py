...
def summarize_with_deepseek(news_items: List[Dict]) -> Optional[str]:
    ...
    system = (
        ...
        "   - **🔬 科技要点**\n"
        "   - **💰 财经要点**\n"
        "4) 每个板块下必须有 3–8 条编号要点（使用 Markdown 编号列表“1. 2. 3.”），任何板块不得为空。\n"
        "5) 每条要点必须严格由三段组成（顺序固定）：\n"
        "   - 第一行：编号 + 加粗小标题（例如：1. **AI巨头动态：……**）\n"
        "   - 第二行：1–2 句中文摘要（尽量精炼）\n"
        "   - 第三行：引用块格式来源行，必须是：> 来源：Axx Ayy Azz（1–4 个编号，空格分隔；不要写链接，不要写 URL）\n"
        "6) 来源必须只引用材料中存在的 Axx；不得编造不存在的编号。\n"
        "7) 全文控制在约 1800–4000 中文字符以内，避免超长。\n"
    )
    ...
    user = (
        ...
        f"强约束：第一行日期必须使用 {report_date}；来源行只写 Axx 编号（空格分隔），不要输出任何链接或 URL。\n\n"
        f"{material}"
    )
