...
def summarize_with_deepseek(news_items: List[Dict]) -> Optional[str]:
    """
    使用 DeepSeek 对过去24小时的科技+财经资讯做汇总，并在每条汇总中附上材料编号来源。
    需要环境变量：
    - DEEPSEEK_API_KEY（必填）
    可选：
    ...
    system = (
        ...
        "并为内部投研/资产配置讨论提供结构化的信息摘要与要点提炼。请严格按指定格式输出中文 Markdown。\n"
        "\n"
        "【输出格式硬性要求，必须严格遵守】：\n"
        "1) 第一行必须是：**📅 YYYY-MM-DD 科技财经日报**（使用加粗，日期为当日日期）。\n"
        "2) 第二块必须是分隔线：---（单独一行）。\n"
        "3) 分隔线后必须依次出现且仅出现以下两个板块（用加粗文本作为板块标题，顺序固定）：\n"
        "   - **🔬 科技要点**\n"
        "   - **💰 财经要点**\n"
        "4) 每个板块标题下允许先写 1 段不带序号的“导语/总括”（可选，1–2 句），导语之后必须开始编号列表。\n"
        "5) 每个板块下必须有 3–8 条编号要点（使用 Markdown 编号列表“1. 2. 3.”），任何板块不得为空。\n"
        "6) 每条要点必须严格由两行组成（顺序固定）：\n"
        "   - 第 1 行：编号 + 加粗标题（例如：1. **AI巨头动态：平台化、安全与法律纠纷**）\n"
        "   - 第 2 行：摘要（1–2 句中文），必须换行呈现，不要与标题同一行。\n"
        "7) 严禁输出任何“来源/出处/链接/URL/Axx 编号”等引用信息；不要在正文中出现 A01 这类编号。\n"
        "8) 在 **💰 财经要点** 中，必须把“财经建议/风险提示（非投资建议）”作为编号列表的第 1 条（也就是 1. 那一条），优先基于财经材料提炼板块/主题层面的关注点与风险提示；只能讨论行业、板块、资产类别或宏观主题，禁止提及具体个股或单一债券；不得给出具体操作指令（买入/卖出/加仓/减仓）。\n"
        "9) 全文控制在约 1800–4000 中文字符以内，避免超长。\n"
    )

    user = (
        f"当前时间（UTC）：{now_utc}\n"
        f"日报日期：{report_date}\n"
        f"请根据以下过去 {HOURS_WINDOW} 小时文章材料生成日报。材料格式为：编号 | 分类 | 来源 | 时间 | 标题 | 链接。\n"
