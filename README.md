...
- **归档**：全文写入 `docs/YYYY-MM-DD.md`，开头即日报内容摘要，随后是五个板块的完整内容；**每条要点后附来源链接**（`🔗 [查看原文](链接)`，开源项目/论文板块附 GitHub/arXiv 链接），链接均取自原始材料，AI 不编造
- **推送**：发摘要版（日报内容摘要 + AI/自动驾驶/机器人/论文每板块最多 4 条一句话概括并附 `[原文]` 链接，开源项目板块最多 10 条仓库全名 + GitHub 链接）+ "查看完整日报"链接（指向 GitHub Pages 上的当日全文）
